...
from run_gcp_benchmark import run_storage_benchmark, run_bigdata_benchmark
...
logger = setup_logger(
    "gcp_multi_benchmark", log_file="../data/results/gcp_multi_benchmark.log"
)
...
def run_ml_multi(num_runs: int = 10, cleanup: bool = True) -> dict:
    ...
    """
    logger.info("=" * 80)
    logger.info(f"Running GCP ML Benchmark - {num_runs} iterations")
    logger.info("=" * 80)

    aggregator = MetricsAggregator(benchmark_type="ml", provider="gcp")

    for i in range(1, num_runs + 1):
        logger.info(f"\n>>> ML Run {i}/{num_runs}")
        try:
            experiment_id = f"ml_run{i:02d}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
            metrics = run_gcp_experiment(cleanup=cleanup, experiment_id=experiment_id)
...
def run_storage_multi(num_runs: int = 10, cleanup: bool = True) -> dict:
    ...
    """
    logger.info("=" * 80)
    logger.info(f"Running GCP Storage Benchmark - {num_runs} iterations")
    logger.info("=" * 80)

    aggregator = MetricsAggregator(benchmark_type="storage", provider="gcp")
...
def run_pyspark_multi(dataset_mode: str, num_runs: int, cleanup: bool = True) -> dict:
    ...
    logger.info("=" * 80)
    logger.info(
        f"Running GCP PySpark Benchmark ({dataset_mode}) - {num_runs} iterations"
    )
    logger.info("=" * 80)

    aggregator = MetricsAggregator(
        benchmark_type="pyspark", provider="gcp", dataset_mode=dataset_mode
    )
...
def run_all_multi(
    ...
    logger.info("=" * 80)
    logger.info("Starting GCP Complete Multi-Run Benchmark Suite")
    ...
    logger.info("=" * 80)
    logger.info("GCP Complete Multi-Run Benchmark Suite Finished!")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run GCP benchmarks with multiple iterations"
    )
    parser.add_argument(
        "--benchmark-type",
